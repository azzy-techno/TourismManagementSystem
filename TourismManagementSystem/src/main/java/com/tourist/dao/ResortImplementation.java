...
public class ResortImplementation implements ResortInterface {
    ...
    public boolean insertData(Resort resort) {
        connect();
        String qry = "INSERT INTO resorts (destination_id, name, description, type, rating) VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean updateData(Resort resort) {
        connect();
        String qry = "UPDATE resorts SET name = ?, description = ?, type = ?, rating = ? WHERE resort_id = ?";
        try (PreparedStatement ptst = con.prepareStatement(qry)) {
            ...
            ptst.setInt(5, resort.getResort_id());
            return ptst.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
