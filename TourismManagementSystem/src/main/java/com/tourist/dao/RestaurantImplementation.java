...
public class RestaurantImplementation implements RestaurantInterface {
    ...
    public boolean insertData(Restaurant restaurant) {
        connect();
        String qry = "INSERT INTO restaurants (destination_id, name, description, cuisine_type, rating) VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean updateData(Restaurant restaurant) {
        connect();
        String qry = "UPDATE restaurants SET name = ?, description = ?, cuisine_type = ?, rating = ? WHERE restaurant_id = ?";
        try (PreparedStatement ptst = con.prepareStatement(qry)) {
            ...
            ptst.setInt(5, restaurant.getRestaurant_id());
            return ptst.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    // Optional: For listing all restaurants for a destination (useful in package page)
